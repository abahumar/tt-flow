...
const TikTokScriptGenerator = () => {
  ...
  const handleDownloadAllImages = async () => {
    ...
    try {
      const indices = Object.keys(stepImages)
        .map(Number)
        .sort((a, b) => a - b);
      for (const idx of indices) {
        await handleDownloadSingleImage(
          stepImages[idx],
          `scene-${idx + 1}_9-16.png`,
        );
        await delay(1500);
      }
    } catch (e) {
...
`;
  ...
  const generateScript = async () => {
    ...
    try {
      const scriptResult = await generateTikTokScript(
        productName,
        additionalContext,
        personaType,
        genre,
      ...
      if (scriptResult.structure && scriptResult.structure.length > 0) {
        ...
        const scene1ImageRef = await handleGenerateSingleScene(
          scriptResult.structure[0].visual_prompt_en,
          0,
        ...
        if (scriptResult.structure.length > 1) {
          ...
          for (let i = 1; i < scriptResult.structure.length; i++) {
            setGenerationStatus(
              `Generating Scene ${i + 1} of ${scriptResult.structure.length}...`,
            );
  ...
  const handleRegenerateScene = async (index: number) => {
    ...
    await handleGenerateSingleScene(
      result.structure[index].visual_prompt_en,
      index,
      result.visual_dna,
      images[0],
      characterRef,
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-pink-500 pb-32">
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-xl tracking-tighter italic">
              Gempak<span className="text-pink-500">Studio</span>
            </span>
          </div>
          <div className="flex items-center gap-2">
            <button
      ...
      <main className="max-w-4xl mx-auto px-4 pt-8">
        <section className="mb-10">
          <div
            className={`transition-all duration-500 ${images.length > 0 ? "bg-gray-900/40 p-4 rounded-2xl border border-white/10" : ""}`}
          >
            {images.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                  {images.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative w-20 h-20 flex-shrink-0 rounded-xl overflow-hidden border border-white/10"
                    >
                      <img src={img} className="w-full h-full object-cover" />
                      {idx === 0 && (
                        <div className="absolute top-0 right-0 bg-pink-500 p-0.5 rounded-bl">
                          <Wand2 size={10} />
                        </div>
                      )}
                    </div>
        ...
        <section className="mb-8">
          <h3 className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-3">
            Pilih Genre Video
          </h3>
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-500 text-xs p-4 rounded-2xl mb-8 flex items-center gap-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/10 pb-4 gap-4">
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <p
                    className={`text-[10px] font-bold tracking-widest uppercase ${genres.find((g) => g.id === genre.toLowerCase())?.color || "text-white"}`}
                  >
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="text-[10px] font-bold px-4 py-2 bg-blue-900/40 border border-blue-500/30 rounded-full hover:bg-blue-900/60 flex items-center gap-2 text-blue-200"
                >
                ...
                <button
                  ...
                  className="text-[10px] font-bold px-4 py-2 bg-pink-900/40 border border-pink-500/30 rounded-full hover:bg-pink-900/60 flex items-center gap-2 text-pink-200 disabled:opacity-50"
                >
                  {downloadingAll ? (
                ...
                >
                  <div className="md:col-span-5">
                    <div className="aspect-[9/16] bg-gray-900 rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl relative">
                      {stepLoading[idx] ? (
                        ...
                        <div className="relative w-full h-full group/image">
                          ...
                          {showSafeZone && (
                            <div className="absolute inset-0 pointer-events-none z-20">
                              <div className="absolute top-0 left-0 right-0 h-[15%] bg-red-500/10 border-b border-red-500/30 flex items-end justify-center pb-1">
                                <span className="text-[8px] text-red-300 font-mono bg-black/50 px-1 rounded">
                                  SEARCH UI
                                </span>
                              </div>
                              ...
                              <div className="absolute top-[15%] bottom-[25%] left-0 right-[15%] border border-green-500/20 m-2 rounded border-dashed flex items-center justify-center">
                                <span className="text-[8px] text-green-500/50 font-mono uppercase tracking-widest opacity-0 group-hover/image:opacity-100 transition-opacity">
                                  Safe Zone
                                </span>
                              </div>
                            </div>
                          ...
                          <div className="absolute top-4 right-4 flex gap-2 z-30 pointer-events-auto">
                            {idx === 0 && (
                              <div className="px-2 py-1 bg-cyan-500 text-black text-[9px] font-black uppercase rounded shadow-lg">
                                Master Ref
                              </div>
                            )}
                            <button
                              onClick={() => handleRegenerateScene(idx)}
                              className="p-1.5 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-pink-600 transition-all border border-white/10"
                          ...
                          <button
                            ...
                            className="absolute bottom-4 right-4 p-3 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-pink-600 transition-all opacity-0 group-hover:opacity-100 z-30 pointer-events-auto shadow-xl shadow-black/50"
                          >
                            <Download size={18} />
                          </button>
                        </div>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-500 bg-gray-900/50">
                          ...
                          <p className="text-[9px] uppercase font-bold opacity-30 tracking-widest mb-4">
                            Waiting Queue
                          </p>
                          <button
                            onClick={() => handleRegenerateScene(idx)}
                            className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-full border border-white/10 flex items-center gap-2 transition-all group/retry"
                          >
                            <RefreshCw
                              size={12}
                              className="group-hover/retry:rotate-180 transition-transform duration-500"
                            />
                      ...
                      <div className="absolute top-4 left-4 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full text-[10px] font-black border border-white/10 z-20">
                        SCENE {idx + 1} • {item.time}
                      </div>
                    </div>
                  </div>
                  <div className="md:col-span-7 space-y-4 pt-4">
                    ...
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          handleUpdateVisualPrompt(idx, e.target.value)
                        }
                      ...
                      <div className="mt-2 pt-2 border-t border-white/5">
                        <details className="group">
                          ...
                          <div className="mt-2 p-2 bg-black/40 rounded border border-white/5 text-[10px] font-mono text-gray-500 leading-relaxed break-words">
                            Generate a high-quality 9:16 vertical photo. SCENE:{" "}
                            {item.visual_prompt_en}. MODEL DNA:{" "}
                            {result.visual_dna}. STYLE: Cinematic, realistic,
                            8k.
                          </div>
                        </details>
                      </div>
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-gray-500 hover:text-white flex items-center gap-2"
                    >
                      {copiedIndex === idx ? (
        ...
        <div className="max-w-md mx-auto">
          {!loading ? (
            <button
              ...
              disabled={
                images.length === 0 || generationStatus.includes("Generating")
              }
            ...
            <div className="w-full bg-gray-900 border border-white/10 py-4 rounded-3xl flex items-center justify-center gap-3">
              ...
              <span className="font-black text-gray-400 text-lg animate-pulse uppercase tracking-widest">
                {generationStatus.includes("Scene")
                  ? generationStatus
                  : "Merangka DNA..."}
              </span>
            </div>
