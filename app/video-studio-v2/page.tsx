...
const PERSONAS = [
  ...
  { id: "hands_only", label: "Tangan Sahaja (Hands Only)", icon: Box },
] as const;
...
export default function VideoStudioV2Page() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Resolve model image: manual upload > settings avatar image
      const settingsAvatarFile =
        !modelPreview && avatarImages[personaType]
          ? avatarImages[personaType]
          : null;
      const effectiveModelImage = uploads.model || settingsAvatarFile || null;
      const isCustomModel = !!modelPreview || !!settingsAvatarFile;

      const body: Record<string, unknown> = {
        mode: "gempak",
        videoType: genre,
        apiKey,
        avatarId: isCustomModel ? "custom" : personaType,
        sceneCount: numScenes,
        includeDialog,
        includeEnglishDialog,
        varyBackground,
        productImage: uploads.prod || null,
        modelImage: effectiveModelImage,
        modelDesc:
          modelDesc ||
          (effectiveModelImage
  ...
  const handleQueueSelected = async () => {
    ...
    try {
      ...
      // Model image: manual upload > settings avatar image
      const settingsAvatarFile =
        !modelPreview && avatarImages[personaType]
          ? avatarImages[personaType]
          : null;
      const effectiveModelRef = uploads.model || settingsAvatarFile || null;
      if (effectiveModelRef) refImages.push(effectiveModelRef);
      console.log(
        "[V2 Queue] refImages:",
        refImages,
        "prod:",
      ...
      <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div>
          ...
          >
            {PERSONAS.map((p) => {
              const Icon = p.icon;
              const hasSettingsAvatar = !!avatarImages[p.id];
              return (
                <button
                  key={p.id}
                ...
                >
                  {hasSettingsAvatar ? (
                    <img
                      src={`/api/upload/${avatarImages[p.id]}`}
                      alt={p.label}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-gray-400">{hookBgColor}</span>
                  </div>
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">
            ...
            <p className="text-[11px] text-gray-400">
              Edit text overlays per scene above. When you click Queue, settings
              are saved. Video will auto-combine after all scenes finish
              generating.
            </p>
